...
class R1Converter(G1Converter):
    """Unitree R1 (24 DOF, 25 bodies) + Dex3-1 converter between IsaacLab and MuJoCo conventions.

    Reuses G1Converter's index helpers; only the mapping constants and the end-effector body
    names differ (R1 arms end at the wrist roll link).
    """

    def __init__(self):  # noqa: D107
        from gear_sonic.envs.manager_env.robots.r1_ordering import (
            R1_ISAACLAB_JOINTS,
...
# robot.type (see modular_tracking_env_cfg.robot_mapping) -> converter class
CONVERTERS = {
    "g1": G1Converter,
...
def get_converter(robot_type: str = "g1") -> IsaacLabMuJoCoConverter:
    """Instantiate the ordering converter for a ``robot.type`` string (defaults to G1)."""
    try:
        return CONVERTERS[robot_type]()
    except KeyError as e:
        raise KeyError(
            f"No order converter registered for robot type {robot_type!r}; known: {sorted(CONVERTERS)}"
        ) from e
